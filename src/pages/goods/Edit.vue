...
export default {
  ...
  async created(){
    const res=await $axios.all([getCategoryList(),getSpecsInfo(),getGoodsById(this.formData.id)]);
    if(res){
      [this.cateList,this.specsList,this.formData]=res;
      this.firstCateChange(this.formData.first_cateid);
      this.changeSpecs(this.formData.specsid,false);
      this.goodsImgList=[{url:this.$imgBaseUrl+this.formData.img}];
